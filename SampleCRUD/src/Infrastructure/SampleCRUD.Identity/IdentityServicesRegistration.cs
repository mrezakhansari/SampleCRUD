...
public static class IdentityServicesRegistration
{
    public static IServiceCollection AddIdentityServices(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddDbContext<SampleCRUDIdentityDbContext>(options =>
        options.UseSqlServer(configuration.GetConnectionString("SampleCRUDConnectionString")));

        services.AddIdentity<ApplicationUser, IdentityRole>()
            .AddEntityFrameworkStores<SampleCRUDIdentityDbContext>().AddDefaultTokenProviders();
